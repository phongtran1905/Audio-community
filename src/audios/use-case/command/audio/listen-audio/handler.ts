...
import { ListenAudioRequest } from './request';
...
@CommandHandler(ListenAudioRequest)
export class ListenAudioHandler implements ICommandHandler<ListenAudioRequest> {
  constructor(
    @Inject(AUDIO_REPOSITORY)
    private repository: IAudioRepository,
  ) {}

  async execute(command: ListenAudioRequest): Promise<void> {
